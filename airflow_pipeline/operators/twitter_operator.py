...
class TwitterOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        with open(self.file_path, 'w') as output_file:
            for pg in TwitterHook(end_time, start_time, query, 'twitter_default').run():
...
if __name__ == '__main__':
    ...
    with DAG(dag_id = 'TwitterTest', start_date=datetime.now()) as dag:
        to = TwitterOperator(file_path=join('datalake/twitter_datascience',f'extract_date={datetime.now().date()}', f"datascience_{datetime.now().date().strftime('%Y%m%d')}.json"),
